...
def get_board_side_length_pixel(corners):
  ...
  for d in range(shape[0]):
    ...
    length.append(np.sqrt((one[1][0] - one[2][0]) **
                          2 + (one[1][1] - one[2][1]) ** 2))
    length.append(np.sqrt((one[2][0] - one[3][0]) **
                          2 + (one[2][1] - one[3][1]) ** 2))
    length.append(np.sqrt((one[3][0] - one[0][0]) **
                          2 + (one[3][1] - one[0][1]) ** 2))
...
def rank_distance(points: np.ndarray):
  ...
  for i in range(len(points)):
    for j in range(len(points)-i-1):
      distance.append(np.sqrt(np.square(
          points[i][0]-points[i+j+1][0])+np.square(points[i][0]-points[i+j+1][0])))
...
def find_board_center_and_windows(corners: np.ndarray, rig: dict):
  ...
  boardA_center = np.array(
      [np.mean(boardA[:, :, 0]), np.mean(boardA[:, :, 1])])
  boardB_center = np.array(
      [np.mean(boardB[:, :, 0]), np.mean(boardB[:, :, 1])])
  boardC_center = np.array(
      [np.mean(boardC[:, :, 0]), np.mean(boardC[:, :, 1])])
  board_length = get_board_side_length_pixel(corners)

  A2A = [get_r_pixel(rig['board_A2A'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_A2A'][1], board_length, rig['board_size'])]
  A2B = [get_r_pixel(rig['board_A2B'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_A2B'][1], board_length, rig['board_size'])]
  A2C = [get_r_pixel(rig['board_A2C'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_A2C'][1], board_length, rig['board_size'])]

  B2A = [get_r_pixel(rig['board_B2A'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_B2A'][1], board_length, rig['board_size'])]
  B2B = [get_r_pixel(rig['board_B2B'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_B2B'][1], board_length, rig['board_size'])]
  B2C = [get_r_pixel(rig['board_B2C'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_B2C'][1], board_length, rig['board_size'])]

  C2A = [get_r_pixel(rig['board_C2A'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_C2A'][1], board_length, rig['board_size'])]
  C2B = [get_r_pixel(rig['board_C2B'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_C2B'][1], board_length, rig['board_size'])]
  C2C = [get_r_pixel(rig['board_C2C'][0], board_length, rig['board_size']), get_r_pixel(
      rig['board_C2C'][1], board_length, rig['board_size'])]

  winA1 = np.array([find_intersect_circle_circle(boardA_center, A2A[0], boardB_center, B2A[0]),
                    find_intersect_circle_circle(
      boardA_center, A2A[0], boardC_center, C2A[0]),
  ...
  winA2 = np.array([find_intersect_circle_circle(boardA_center, A2A[1], boardB_center, B2A[1]),
                    find_intersect_circle_circle(
      boardA_center, A2A[1], boardC_center, C2A[1]),
  ...
  winB1 = np.array([find_intersect_circle_circle(boardA_center, A2B[0], boardB_center, B2B[0]),
                    find_intersect_circle_circle(
      boardA_center, A2B[0], boardC_center, C2B[0]),
  ...
  winB2 = np.array([find_intersect_circle_circle(boardA_center, A2B[1], boardB_center, B2B[1]),
                    find_intersect_circle_circle(
      boardA_center, A2B[1], boardC_center, C2B[1]),
  ...
  winC1 = np.array([find_intersect_circle_circle(boardA_center, A2C[0], boardB_center, B2C[0]),
                    find_intersect_circle_circle(
      boardA_center, A2C[0], boardC_center, C2C[0]),
  ...
  winC2 = np.array([find_intersect_circle_circle(boardA_center, A2C[1], boardB_center, B2C[1]),
                    find_intersect_circle_circle(
      boardA_center, A2C[1], boardC_center, C2C[1]),
    ...
    if self.save_center:
      if os.path.exists(self.config_folder_path+'\\config_behavior_rig.toml'):
        with open(self.config_folder_path+'\\config_behavior_rig.toml', 'a') as f:
          new_toml_string = toml.dump(
              {'recorded_center': self.circle_center}, f)
          print('saved')
...
class Gaze_angle:
  def __init__(self, config_folder_path, gazePoint=0.5725, main_config_path=None):
    ...
    self.inner_r_pixel = get_r_pixel(
        self.inner_r, corners_pixel, self.board_size)
    self.outer_r_pixel = get_r_pixel(
        self.outer_r, corners_pixel, self.board_size)
  ...
  def __call__(self, root_path, cutoff=0.9, save=True):
    ...
    pose.loc[pose.leftear.likelihood < cutoff, [
        ('leftear', 'x'), ('leftear', 'y')]] = np.nan
    pose.loc[pose.rightear.likelihood < cutoff, [
        ('rightear', 'x'), ('rightear', 'y')]] = np.nan
    pose.loc[pose.snout.likelihood < cutoff, [
        ('snout', 'x'), ('snout', 'y')]] = np.nan
    pose.loc[pose.tailbase.likelihood < cutoff, [
    ...
    for i, dis in enumerate(distance):
      # TODO: much faster to use numpy instead of for loop
      # create a logical array: is_bad = np.logical_or(distance<self.inner_r_pixel, distance>self.outer_r_pixel)
      # inner_left[is_bad] = np.nan
      # etc.
    ...
    # in window
    A_right = np.sum(is_in_window(
        outer_right, self.windowA, self.circle_center))
    B_right = np.sum(is_in_window(
        outer_right, self.windowB, self.circle_center))
    C_right = np.sum(is_in_window(
        outer_right, self.windowC, self.circle_center))
    ...
    winPreference_right = ((A_right + B_right + C_right) /
                           np.sum(pd.notna(outer_right))) * 2*np.pi/windows_arc
    winPreference_left = ((A_left + B_left + C_left) /
                          np.sum(pd.notna(outer_left))) * 2*np.pi/windows_arc
    winPreference_body = ((A_body + B_body + C_body) /
                          np.sum(pd.notna(arc_body))) * 2*np.pi/windows_arc
  ...
  def _get_window_angle(self):
    ...
    window_A_angle = np.sort(np.arctan2(
        window_A_center[:, 1], window_A_center[:, 0]))
    window_B_angle = np.sort(np.arctan2(
        window_B_center[:, 1], window_B_center[:, 0]))
    window_C_angle = np.sort(np.arctan2(
        window_C_center[:, 1], window_C_center[:, 0]))

    window_A_angle_extra = np.array(
        [i - 2 * np.pi if i > 0 else i + 2 * np.pi for i in window_A_angle])
    window_B_angle_extra = np.array(
        [i - 2 * np.pi if i > 0 else i + 2 * np.pi for i in window_B_angle])
    window_C_angle_extra = np.array(
        [i - 2 * np.pi if i > 0 else i + 2 * np.pi for i in window_C_angle])
  ...
  def save(self, path, data):
    ...
    if isinstance(data, pd.DataFrame):
      data.to_csv(os.path.join(path, 'gaze_angle_%d.csv' %
                               int(self.gazePoint*180/np.pi)))
    else:
      filename = os.path.join(path, 'gaze_angle_%d.mat' %
                              int(self.gazePoint*180/np.pi))
  ...
  def plot(self, things: dict, savepath=None, show=False):
    ...
    keys = ['inner_left', 'inner_right',
            'outer_left', 'outer_right', 'body_position']
    row = 4
    col = 2
    ...
    for i in range(len(keys)):
      ...
      plt.axvspan(xmin=self.windows[0][0][0], xmax=self.windows[0]
                  [0][1], facecolor='orange', alpha=0.3)
      plt.axvspan(
          xmin=self.windows[1][0][0], xmax=self.windows[1][0][1], facecolor='red', alpha=0.3)
      plt.axvspan(xmin=self.windows[2][0][0], xmax=self.windows[2]
                  [0][1], facecolor='magenta', alpha=0.3)
      plt.axvspan(xmin=self.windows[0][1][0], xmax=self.windows[0]
                  [1][1], facecolor='orange', alpha=0.3)
      plt.axvspan(
          xmin=self.windows[1][1][0], xmax=self.windows[1][1][1], facecolor='red', alpha=0.3)
      plt.axvspan(xmin=self.windows[2][1][0], xmax=self.windows[2]
                  [1][1], facecolor='magenta', alpha=0.3)
      handles = [Rectangle((0, 0), 1, 1, color=c, ec="k", alpha=0.3)
                 for c in ['orange', 'red', 'magenta']]
      labels = ["window A", "window B", "window C"]
      plt.legend(handles, labels)
      plt.title("Gaze point density of %s " % list(keys)[i])
      plt.hist(x=self.double(
          np.array(things[list(keys)[i]])), bins=60, density=False)
    plt.suptitle(self.title_name + ' gaze angle %d' %
                 float(self.gazePoint*180/np.pi), fontsize=30)
    ...
    table = plt.table(cellText=cell_text, colLabels=stats.columns,
    ...
    if savepath:
      plt.savefig(os.path.join(savepath, 'gaze', 'gaze_angle_%d.jpg' %
                               float(self.gazePoint*180/np.pi)))
...
if __name__ == '__main__':
  ...
  for video in videos:
    if not video.startswith('.'):
      config_folder_path2 = os.path.join(
          videopaths, video, 'config_behavior_rig.toml')
      gaze_model = Gaze_angle(main_config_path=config_folder_path,
                              config_folder_path=config_folder_path2, gazePoint=0)
      result = gaze_model(os.path.join(videopaths, video), save=True)
      gaze_model.plot(result, savepath=os.path.join(
          videopaths, video), show=False)
